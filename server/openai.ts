...
export async function generateHistoricalPortrait(params: GeneratePortraitParams): Promise<{ url: string }> {
  const { imageBase64, yearWar, side, rank, branch, extraDetails, artStyle } = params;
  ...
  const stylePrompts = {
    ...
    watercolor: "Create an old watercolor portrait in the style of historical military watercolor paintings. Use traditional watercolor techniques with dramatic lighting and muted, earthy palettes giving a grounded natural feel. Apply an authentic old parchment/paper texture background. The style should evoke vintage military watercolor portraits from historical archives."
  };
